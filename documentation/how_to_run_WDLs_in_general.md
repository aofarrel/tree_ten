...
* A WDL executor
* An internet connection (some deps are downloaded at runtime)
...
#### What if my institute doesn't allow me to use Docker for security reasons?
You have multiple options:   
a) Read [Dockstore's documentation on Docker alternatives](https://docs.dockstore.org/en/stable/advanced-topics/docker-alternatives.html)  
b) Tree Nine is quite scalable; you might be able to get away with running it locally if you have only a few (<200ish) samples   
c) Arm-wrestle your sysadmin   

Although we cannot support non-Docker versions of this workflow, we happily accept documentation contributions from people who have managed to get Tree Ten running on not-Docker.
...
* TOIL (WDL support is in beta and not tested with these workflows)
